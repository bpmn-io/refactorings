...
export default class OpenAIClient {
  ...
  async getToolCalls(element, tools, userPrompt) {
    ...
    try {
      const request = {
        messages: [
          ...
          {
            'role': 'user',
            'content': userPrompt || this._getUserPrompt(element)
          }
        ],
        model: this._model,
        seed: this._seed,
      ...
      const response = await this._createChatCompletion(request);
      ...
      try {
        let { templates } = JSON.parse(message, null, 2);

        // filter out fallback templates
        templates = templates.filter(template => !template.startsWith('__fallback'))

        // filter out REST if other templates are available
        // TODO: we could also assign ranking to templates instead of matching by name+
        // REST templates would be ranked lower
        // TODO: do the same for generic webhook templates
        if (templates.some(template => !template.toLowerCase().includes('http'))) {
          templates = templates.filter(template => !template.toLowerCase().includes('http'));
        }
