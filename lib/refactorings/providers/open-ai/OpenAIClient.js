...
export default class OpenAIClient {
  ...
  async getToolCalls(element, tools) {
    try {
      ...
      console.log('Tools:', getTools(message));

      return getTools(message);
    } catch (error) {
...
function getTools(message) {
  try {
    const tools = JSON.parse(message);

    return Object.entries(tools).reduce((tools, [ tool, confidence ]) => {
      if (confidence < 0.5) {
        return tools;
      }
