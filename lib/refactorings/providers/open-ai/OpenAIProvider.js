...
export default class OpenAIProvider {
  ...
  async getRefactorings(elements) {
    ...
    if (this._debug) {
      console.log('[OpenAIProvider#getRefactorings] tools that can execute:', JSON.stringify(tools, null, 2));
    }
    ...
    if (this._debug) {
      console.log('[OpenAIProvider#getRefactorings] tool calls:', JSON.stringify(toolCalls, null, 2));
    }

    const refactorings = toolCalls.reduce((refactorings, toolCall) => {
  ...
  _getToolCalls(element, tools) {
    return this._openAIClient.getToolCalls(element, tools);
  }
}
