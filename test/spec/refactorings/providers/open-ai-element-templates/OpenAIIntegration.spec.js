...
import bluePrismElementTemplates from '../../../../fixtures/element-templates/blue-prism-connector.json';
import slackElementTemplates from '../../../../fixtures/element-templates/slack-connector.json';
import whatsAppElementTemplates from '../../../../fixtures/element-templates/whatsapp-connector.json';
...
(testOpenai ? describe.only : describe.skip)('OpenAI integration (element templates)', function() {
  ...
  describe('Camunda connector templates', function() {
  ...
  describe('custom connector templates', function() {
    ...
    beforeEach(bootstrapModeler(diagramXML, {
      ...
      elementTemplates: [
        ...bluePrismElementTemplates.map(elementTemplate => addKeywords(elementTemplate, [ 'add to queue' ])),
        ...slackElementTemplates,
        ...whatsAppElementTemplates
      ],
      refactorings: {
        openai: {
          createChatCompletion: (...args) => openai.chat.completions.create(...args)
     ...
     */
    expectRefactorings('bpmn:Task', 'Add to Blue Prism', [
      createRefactoring('io.camunda.connectors.BluePrism.v1')
    ]);
     ...
     * Matching keywords.
     */
    expectRefactorings('bpmn:Task', 'Add to queue', [
      createRefactoring('io.camunda.connectors.BluePrism.v1')
    ]);
...
function addKeywords(elementTemplate, keywords) {
  return {
    ...elementTemplate,
    metadata: {
      keywords
